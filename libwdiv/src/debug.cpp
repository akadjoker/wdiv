...
size_t Debug::disassembleInstruction(const Code &chunk, size_t offset)
{
    ...
    case OP_CALL_NATIVE:
    {
        ...
        printf("%-16s %4u '%s' (%u args)\n",
               "OP_CALL_NATIVE", (unsigned)nameIdx, nm, (unsigned)argCount);
...
size_t Debug::constantInstruction(const char *name, const Code &chunk, size_t offset)
{
...
size_t Debug::constantNameInstruction(const char *name, const Code &chunk, size_t offset)
{
    if (offset + 1 >= chunk.count)
    {
        printf("%s <truncated>\n", name);
...
size_t Debug::byteInstruction(const char *name, const Code &chunk, size_t offset)
{
    if (offset + 1 >= chunk.count)
    {
        printf("%s <truncated>\n", name);
...
size_t Debug::jumpInstruction(const char *name, int sign, const Code &chunk, size_t offset)
{
    ...
    uint16 jump = (uint16)(chunk.code[offset + 1] << 8) | (uint16)chunk.code[offset + 2];
