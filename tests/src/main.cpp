...
void endTestFile()
{
    ...
    else
    {
        printf("❌ %s: %d passed, %d failed\n",
               currentTestFile.c_str(), testsPassed, testsFailed);
    }
}
...
static Value native_assert_eq(Interpreter *vm, int argc, Value *args)
{
    ...
    Value a = args[0];
    Value b = args[1];
    const char *name = args[2].isString() ? args[2].asString()->chars() : "equality";
    ...
    else
    {
        ...
        char reason[256];
        snprintf(reason, sizeof(reason), "expected '%s', got '%s'", valueToString(b).c_str(), valueToString(a).c_str());
...
static Value native_assert(Interpreter *vm, int argc, Value *args)
{
    ...
    const char *name = args[1].isString() ? args[1].asString()->chars() : "assertion";
